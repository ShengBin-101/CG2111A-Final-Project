...
void handleMessage(TPacket *packet)
{
	...
	strncpy(data, packet->data, MAX_STR_LEN);
	printf("Ultrasonic Reading is: %s\n", data);
}

void handleUltrasonicMessage(TPacket *packet)
{
	char data[65];
	printf("UART ULTRASONIC MESSAGE PACKET\n");
	data[0] = NET_ULTRASONIC_MESSAGE_PACKET;
	memcpy(&data[1], packet->params, sizeof(packet->params));
	sendNetworkData(data, sizeof(data));
}

void handleColorMessage(TPacket *packet)
{
	char data[65];
	printf("UART COLOR MESSAGE PACKET: %s\n", packet->params);
	data[0] = NET_COLOR_MESSAGE_PACKET;
	memcpy(&data[1], packet->params, sizeof(packet->params));
	sendNetworkData(data, sizeof(data));
}

void handleModeMessage(TPacket *packet)
{
	char data[65];
	printf("UART MANUAL MODE PACKET\n");
	data[0] = NET_MANUALMODE_PACKET;
	memcpy(&data[1], packet->params, sizeof(packet->params));
	sendNetworkData(data, sizeof(data));
}

void handleBuzzerMessage(TPacket *packet)
{
	char data[65];
	printf("UART BUZZER PACKET\n");
	data[0] = NET_BUZZER_PACKET;
	memcpy(&data[1], packet->params, sizeof(packet->params));
	sendNetworkData(data, sizeof(data));
}
...
void handleUARTPacket(TPacket *packet)
{
	switch(packet->packetType)
	{
		...
		case PACKET_TYPE_ULTRASONIC_MESSAGE:
				handleUltrasonicMessage(packet);
			break;
		case PACKET_TYPE_COLOR_MESSAGE:
				handleColorMessage(packet);
			break;
		case PACKET_TYPE_MESSAGE:
				handleMessage(packet);
			break;
		case PACKET_TYPE_BUZZER:
				handleBuzzerMessage(packet);
			break;
		case PACKET_TYPE_MANUAL:
				handleModeMessage(packet);

		case PACKET_TYPE_HELLO:
				printf("HELLO PACKET FROM ARDUINO\n");
			break;
	}
}
...
void uartSendPacket(TPacket *packet)
{
	char buffer[PACKET_SIZE];
	 //len refers to the size of the TComms packet
...
void handleCommand(void *conn, const char *buffer)
{
	...
	switch(cmd)
	{
		...
		case 'U':
			...
			uartSendPacket(&commandPacket);
			break;
		// BUZZER_1 (start_tone - battle)
		case 'i':
		case 'I':
			commandPacket.command = COMMAND_BUZZER_1;
			uartSendPacket(&commandPacket);
			break;
		// BUZZER_2 (low_hp - beeping)
		case 'o':
		case 'O':
			commandPacket.command = COMMAND_BUZZER_2;
			uartSendPacket(&commandPacket);
			break;
		// BUZZER_3 (caught - pokemon)
		case 'p':
		case 'P':
			commandPacket.command = COMMAND_BUZZER_3;
			uartSendPacket(&commandPacket);
			break;
		// BUZZER_4 (celebrate - pokemon centre)
		case '{':
		case '[':
			commandPacket.command = COMMAND_BUZZER_4;
			uartSendPacket(&commandPacket);
			break;
		// BUZZER_5 (tokyo_drift)
		case ']':
		case '}':
			commandPacket.command = COMMAND_BUZZER_5;
			uartSendPacket(&commandPacket);
			break;
		// STABLE MODE
		case 'm':
		case 'M':
			commandPacket.command = COMMAND_TOGGLE_MANUAL;
			uartSendPacket(&commandPacket);
			break;
